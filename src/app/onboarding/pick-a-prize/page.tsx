    ...
    <OnboardingScaffold
      ...
      next={selectedSide ? (
        <Button
          ...
          variant="ghost"
          className="rounded-full h-12 w-12 p-0 bg-[#320E3B] dark:bg-black/30 hover:bg-[#320E3B]/90 dark:hover:bg-black/50 text-white shadow-lg transition-all hover:scale-105 backdrop-blur-sm border-[rgba(50,14,59,0.4)] dark:border-white/20 animate-in fade-in duration-500"
          aria-label="Next"
          disabled={isFinalizing || authLoading}
        >
          {isFinalizing || authLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : <ArrowRight className="h-6 w-6" />}
        </Button>
      ) : undefined}
    >
