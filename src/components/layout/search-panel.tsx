...
const LABEL: React.CSSProperties = { fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(193,154,107,0.55)', margin: '0 0 8px 2px' };
const CARD: React.CSSProperties = { borderRadius: 14, border: '0.5px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.025)', overflow: 'hidden' };
    ...
    <Sheet open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      ...
      >
        ...
        <div className="p-4 pt-5 pr-12">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, height: 42, padding: '0 14px', borderRadius: 999, border: '0.5px solid rgba(193,154,107,0.3)', background: 'rgba(193,154,107,0.06)' }}>
            <Search className="h-[18px] w-[18px]" style={{ color: 'rgba(193,154,107,0.7)' }} />
            <input
              autoFocus
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Search poses, challenges, history…"
              style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: 'rgba(255,245,230,0.92)', fontSize: 14 }}
            />
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: on ? 'rgba(193,154,107,0.85)' : 'transparent',
                    color: on ? '#1a1210' : 'rgba(255,245,230,0.6)',
                    border: on ? 'none' : '0.5px solid rgba(193,154,107,0.25)',
        ...
        <div className="px-4 pb-8">
            ...
            <>
              ...
              <div style={{ ...CARD, marginBottom: 16 }}>
                {challengeHits.map((c, i) => {
                  const inner = (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: i === 0 ? 'none' : '0.5px solid rgba(255,255,255,0.06)' }}>
                      <Trophy className="h-5 w-5" style={{ color: 'rgba(193,154,107,0.85)', flexShrink: 0 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255,245,230,0.9)', margin: 0 }}>{c.name}</p>
                        <p style={{ fontSize: 11, color: 'rgba(255,245,230,0.4)', margin: '1px 0 0', fontStyle: 'italic' }}>{c.category}</p>
                      </div>
                      {c.detailLink !== '#' && <ChevronRight className="h-4 w-4" style={{ color: 'rgba(255,245,230,0.3)' }} />}
                    </div>
                  );
                  return c.detailLink !== '#'
                    ? <Link key={c.id} href={c.detailLink} onClick={onClose} className="block hover:bg-[rgba(193,154,107,0.05)]">{inner}</Link>
          ...
          {showHistory && (
            <>
              ...
              <div style={CARD}>
                {historyHits.map((a, i) => (
                  <Link key={a.id} href={`/analysis/${a.id}`} onClick={onClose} className="block hover:bg-[rgba(193,154,107,0.05)]">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: i === 0 ? 'none' : '0.5px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p className="truncate" style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255,245,230,0.9)', margin: 0 }}>{a.identifiedPose}</p>
                        <p style={{ fontSize: 11, color: 'rgba(255,245,230,0.4)', margin: '1px 0 0', fontStyle: 'italic' }}>{a.createdAt ? format(a.createdAt.toDate(), 'PP') : ''}</p>
                      </div>
                      <span style={{ background: 'rgba(193,154,107,0.2)', color: 'rgba(193,154,107,0.92)', borderRadius: 999, padding: '2px 9px', fontWeight: 500, fontSize: 11, flexShrink: 0 }}>
