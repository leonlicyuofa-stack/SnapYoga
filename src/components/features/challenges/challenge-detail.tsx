...
function SectionHead({ children }: { children: React.ReactNode }) {
  ...
  return (
    <p style={{ fontSize: 10, letterSpacing: '0.24em', textTransform: 'uppercase', color: isDark ? 'rgba(193,154,107,0.6)' : '#320E3B', fontWeight: 600, fontFamily: FONT_CASUAL, margin: '26px 0 12px' }}>{children}</p>
  );
}
...
export function ChallengeDetail(props: ChallengeDetailProps) {
  ...
  const card = isDark ? 'rgba(13,20,30,0.5)' : 'rgba(255,255,255,0.12)';
  const cardBorder = isDark ? 'rgba(193,154,107,0.18)' : 'rgba(255,255,255,0.40)';
  const softBtn = isDark ? 'rgba(193,154,107,0.06)' : 'rgba(255,255,255,0.10)';
      ...
      <div style={{ padding: '16px 14px 28px', maxWidth: 640, margin: '0 auto' }}>
        ...
        {isActive && dayInChallenge && (
          <div style={{ marginTop: 14, borderRadius: 16, border: `0.5px solid ${cardBorder}`, background: card, padding: '12px 14px' }}>
            ...
            <div style={{ height: 8, background: txt(0.08), borderRadius: 5, overflow: 'hidden', marginTop: 8 }}>
              <div style={{ height: '100%', width: `${pct}%`, background: isDark ? 'linear-gradient(90deg, rgba(193,154,107,0.7), rgba(210,180,110,0.95))' : 'linear-gradient(90deg, rgba(50,14,59,0.55), rgba(50,14,59,0.85))', borderRadius: 5 }} />
            </div>
          </div>
        )}

        {/* Description */}
        <p style={{ fontSize: 13, lineHeight: 1.6, color: txt(0.72), margin: '16px 2px', fontFamily: FONT_PANCAKE }}>{props.description}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          <button
            ...
            style={{
              ...
              background: isCompletedToday ? 'rgba(99,196,122,0.12)' : (isDark ? 'linear-gradient(180deg, rgba(214,178,130,0.92), rgba(193,154,107,0.82))' : '#320E3B'),
          ...
          <Link href={props.inviteLink} style={{ height: 48, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: FONT_PANCAKE, fontSize: 15, letterSpacing: '0.04em', border: `0.5px solid ${acc(0.3)}`, background: softBtn, color: txt(0.85), textDecoration: 'none' }}>
        ...
        {props.friends.length > 0 ? (
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {props.friends.map(f => (
              <div key={f.id} style={{ flex: '1 1 140px', borderRadius: 14, border: `0.5px solid ${cardBorder}`, background: card, padding: '14px 8px', textAlign: 'center' }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', margin: '0 auto 8px', background: acc(0.2), border: `2px solid ${acc(0.3)}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: acc(0.95), fontSize: 16 }}>{f.name.charAt(0)}</div>
                <div style={{ fontFamily: FONT_PANCAKE, fontSize: 14, color: txt(0.92) }}>{f.name}</div>
                <div style={{ fontSize: 9, color: acc(0.7), marginTop: 3, textTransform: 'uppercase', letterSpacing: 0.5 }}>Day {f.daysIn}</div>
              </div>
            ))}
          </div>
        ) : (
          <p style={{ fontSize: 12, color: txt(0.5), fontStyle: 'italic', textAlign: 'center' }}>Invite friends to join you in this challenge!</p>
        )}
        ...
        {props.weeklyTutorials.map(week => (
          <div key={week.week}>
            <p style={{ fontFamily: FONT_PANCAKE, fontSize: 16, color: txt(0.92), margin: '14px 0 8px' }}>Week {week.week} · {week.title}</p>
            {week.videos.map(video => (
              <div key={video.day} style={{ borderRadius: 14, border: `0.5px solid ${cardBorder}`, background: card, overflow: 'hidden', marginBottom: 10 }}>
                ...
                <div style={{ padding: '10px 12px' }}>
                  <span style={{ fontSize: 8, letterSpacing: 1, textTransform: 'uppercase', color: isDark ? 'rgba(214,178,130,0.9)' : '#320E3B', background: isDark ? 'rgba(193,154,107,0.14)' : 'rgba(50,14,59,0.10)', border: `0.5px solid ${acc(0.25)}`, borderRadius: 999, padding: '2px 8px' }}>Day {video.day}</span>
                  <div style={{ fontFamily: FONT_PANCAKE, fontSize: 15, color: txt(0.92), margin: '6px 0 3px' }}>{video.title}</div>
                  <div style={{ fontSize: 11, color: txt(0.6), lineHeight: 1.5 }}>{video.description}</div>
                </div>
              </div>
            ))}
          </div>
